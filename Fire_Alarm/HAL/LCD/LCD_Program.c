...
void LCD_VoidInit()
{
    ...
    DIO_VoidSetPinDirection(CFG_PORT, PIN5, OUTPUT);    // Pin5 : Rs
    DIO_VoidSetPinDirection(CFG_PORT, PIN6, OUTPUT);    // Pin6 : Rw
    DIO_VoidSetPinDirection(CFG_PORT, PIN7, OUTPUT);    // Pin7 : En
...
void LCD_VoidWriteCommand(u8 Command)
{
    ...
    DIO_VoidSetPinValue(CFG_PORT, PIN5, LOW);

    // Set Rw = 0 to enable write mode
    DIO_VoidSetPinValue(CFG_PORT, PIN6, LOW);

    // Write command from the port
    DIO_VoidSetPortValue(DATA_PORT, Command);

    // Set En = 1 to enable the LCD
    DIO_VoidSetPinValue(CFG_PORT, PIN7, HIGH);    // set high level (raising edge)
    _delay_ms(1);                                 // delay
    DIO_VoidSetPinValue(CFG_PORT, PIN7, LOW);     // set low level (falling edge)
    _delay_ms(1);                                 // delay
}
...
void LCD_VoidWriteChar(u8 Character)
{
    ...
    DIO_VoidSetPinValue(CFG_PORT, PIN5, HIGH);
